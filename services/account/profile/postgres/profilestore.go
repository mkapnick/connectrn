...
const (
	CreateProfileQuery               = `INSERT INTO profile (id, account_id, first_name, last_name, phone_number, created_at, updated_at) VALUES (:id, :account_id, :first_name, :last_name, :phone_number, :created_at, :updated_at);`
	UpdateProfileQuery               = `UPDATE profile SET first_name = :first_name, last_name = :last_name, phone_number = :phone_number, preferences = :preferences, updated_at = :updated_at WHERE id = :id;`
	FetchProfileQuery                = `SELECT * FROM profile WHERE id = $1;`
	FetchProfileByAccountIDQuery     = `SELECT * FROM profile WHERE account_id = $1`
	FetchAllProfilesByConditionQuery = `SELECT profile.id, profile.first_name, profile.last_name, profile.phone_number, account.email FROM profile JOIN account ON profile.account_id = account.id WHERE`
...
func (s *profileStore) FetchAllByCondition(whereCondition string) ([]*profile.Profile, error) {
	var p []*profile.Profile

	query := fmt.Sprintf("%s %s", FetchAllProfilesByConditionQuery, whereCondition)
	err := s.db.Select(&p, query)
